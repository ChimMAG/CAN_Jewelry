...
namespace canjewelry.src.utils
{
    public static class VariantExtensions
    {
            ...
            if (stackWithAttributes.Attributes[CANBasePSContainer.PSAttributes] is ITreeAttribute tree && block.Attributes["variantTextures"].Exists)
            {
                foreach (var pair in block.Attributes["variantTextures"].AsObject<Dictionary<string, string[]>>())
                {
                    ...
                    foreach (var attr in tree)
                    {
                        ...
                        foreach (string texPath in texPaths.Reverse())
                        { // Reverse to start from the end texture paths (patched textures), first one serving as default
        ...
        /// </summary>
        public static void ApplyVariantTextures(this Shape shape, CANBEBasePSContainer pscontainer)
        {
        ...
        /// </summary>
        public static ItemStack GetVariantStack(this CANBEBasePSContainer entity)
        {
            var stack = new ItemStack(entity.Block);
            if (entity.VariantAttributes.Count != 0)
            {
                stack.Attributes[CANBasePSContainer.PSAttributes] = entity.VariantAttributes;
            }
